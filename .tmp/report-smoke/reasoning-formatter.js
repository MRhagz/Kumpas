...
exports.formatReasoningSummary = formatReasoningSummary;
exports.formatReasoningSummaries = formatReasoningSummaries;
const DEFAULT_MAX_LENGTH = 700;
const PLACEHOLDER_SUMMARY = "A detailed reasoning summary is not available for this recommendation. Review the alignment score, key signals, and source references before presenting it to the student.";
function formatReasoningSummary(recommendation, options = {}) {
...
function formatReasoningSummaries(recommendations, options = {}) {
    return recommendations.map((recommendation) => formatReasoningSummary(recommendation, options));
}
function buildConcerns(recommendation) {
    const concerns = [];
    if (recommendation.status === "degraded") {
        concerns.push(recommendation.degradedReason ??
            "This recommendation was generated with partial reasoning coverage.");
    }
    if (recommendation.status === "incomplete") {
        concerns.push(recommendation.incompleteReason ??
...
function truncateAtSentenceBoundary(value, maxLength) {
